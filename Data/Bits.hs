...
instance Bits Int where
    ...
    (I# x#) `unsafeShiftL` (I# i#) = I# (x# `iShiftL#` i#)
    (I# x#) `shiftR` (I# i#)       = I# (x# `iShiftRA#` i#)
    (I# x#) `unsafeShiftR` (I# i#) = I# (x# `iShiftRA#` i#)

    {-# INLINE rotate #-}       -- See Note [Constant folding for rotate]
    (I# x#) `rotate` (I# i#) =
        I# ((x# `iShiftL#` i'#) `orI#` (x# `iShiftRL#` (wsib -# i'#)))
      where
        !i'# = i# `andI#` (wsib -# 1#)
...
instance Bits Word where
    ...
    (W# x#) .&.   (W# y#)    = W# (int2Word# ((word2Int# x#) `andI#` (word2Int# y#)))
    (W# x#) .|.   (W# y#)    = W# (int2Word# ((word2Int# x#) `orI#` (word2Int# y#)))
    (W# x#) `xor` (W# y#)    = W# (int2Word# ((word2Int# x#) `xorI#` (word2Int# y#)))
    complement (W# x#)       = W# (int2Word# ((word2Int# x#) `xorI#` (word2Int# mb#)))
        where !(W# mb#) = maxBound
    (W# x#) `shift` (I# i#)
        | isTrue# (i# >=# 0#)      = W# (int2Word# ((word2Int# x#) `iShiftL#` i#))
        | otherwise                = W# (int2Word# ((word2Int# x#) `iShiftRL#` negateInt# i#))
    (W# x#) `shiftL` (I# i#)       = W# (int2Word# ((word2Int# x#) `iShiftL#` i#))
    (W# x#) `unsafeShiftL` (I# i#) = W# (int2Word# ((word2Int# x#) `iShiftL#` i#))
    (W# x#) `shiftR` (I# i#)       = W# (int2Word# ((word2Int# x#) `iShiftRL#` i#))
    (W# x#) `unsafeShiftR` (I# i#) = W# (int2Word# ((word2Int# x#) `iShiftRL#` i#))
    (W# x#) `rotate` (I# i#)
        | isTrue# (i'# ==# 0#) = W# x#
        | otherwise  = W# (int2Word# (((word2Int# x#) `iShiftL#` i#) `orI#` ((word2Int# x#) `iShiftRL#` i#)))
        where
        !i'# = i# `andI#` (wsib -# 1#)
        !wsib = WORD_SIZE_IN_BITS#  {- work around preprocessor problem (??) -}
    bitSizeMaybe i           = Just (finiteBitSize i)
...
instance Bits Integer where
    ...
    (Z# x#) .&.   (Z# y#)          = Z# (x# `andI#` y#)
    (Z# x#) .|.   (Z# y#)          = Z# (x# `orI#`  y#)
    (Z# x#) `xor` (Z# y#)          = Z# (x# `xorI#` y#)
    complement (Z# x#)             = Z# (notI# x#)
    (Z# x#) `shift` (I# i#)
        | isTrue# (i# >=# 0#)      = Z# (x# `iShiftL#` i#)
        | otherwise                = Z# (x# `iShiftRA#` negateInt# i#)
    (Z# x#) `shiftL` (I# i#)       = Z# (x# `iShiftL#` i#)
    (Z# x#) `unsafeShiftL` (I# i#) = Z# (x# `iShiftL#` i#)
    (Z# x#) `shiftR` (I# i#)       = Z# (x# `iShiftRA#` i#)
    (Z# x#) `unsafeShiftR` (I# i#) = Z# (x# `iShiftRA#` i#)

    {-# INLINE rotate #-}       -- See Note [Constant folding for rotate]
    (Z# x#) `rotate` (I# i#) =
        Z# ((x# `iShiftL#` i'#) `orI#` (x# `iShiftRL#` (wsib -# i'#)))
      where
        !i'# = i# `andI#` (wsib -# 1#)
        !wsib = WORD_SIZE_IN_BITS#   {- work around preprocessor problem (??) -}
    bitSize i              = errorWithoutStackTrace "Data.Bits.bitSize(Integer)"
