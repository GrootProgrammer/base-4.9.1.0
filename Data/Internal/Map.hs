...
(!?) m k = Data.Map.Internal.lookup k m
-- #if __GLASGOW_HASKELL__
-- {-# INLINE (!?) #-}
-- #endif
-- 
...
-- --
-- -- > Data.Map.Internal.null (empty)           == True
-- -- > Data.Map.Internal.null (singleton 1 'a') == False
-- 
null :: Map k a -> Bool
null (Assocs []) = True
null (Assocs _ ) = False
...
lookup key (Assocs kas) = P.lookup key kas
-- lookup = go
--   where
--     go !_ Tip = Nothing
...
member key (Assocs kas) = case P.filter ((== key) . fst) kas of
    [] -> False
    _ -> True
-- member = go
--   where
...
find key (Assocs ((k, a):kas)) = if k == key then a else find key (Assocs kas)
find _ (Assocs []) = error "find: element not found"
...
findWithDefault a k m = case Data.Map.Internal.lookup k m of
    Just v -> v
    Nothing -> a
-- findWithDefault = go
--   where
...
insert k a (Assocs kas) = Assocs (L.insertBy keycmp (k, a) kas')
  where
    kas' = L.filter ((/= k) . fst) kas
...
insertWith f k a m = case Data.Map.Internal.lookup k m of
...
insertWithKey f k a m = case Data.Map.Internal.lookup k m of
  Nothing -> insert k a m
...
insertLookupWithKey :: Ord k => (k -> a -> a -> a) -> k -> a -> Map k a
                    -> (Maybe a, Map k a)
insertLookupWithKey f k a m = case Data.Map.Internal.lookup k m of
...
delete k (Assocs kas) = Assocs (filter ((/= k) . fst) kas)
-- delete = go
--   where
...
union m1 (Assocs []) = m1
union m1 (Assocs ((k,a):kas)) = case Data.Map.Internal.lookup k m1 of
...
unionWith f m1 (Assocs ((k2,a2):kas)) = case Data.Map.Internal.lookup k2 m1 of
...
unionWithKey f m1 (Assocs ((k2,a2):kas)) = case Data.Map.Internal.lookup k2 m1 of
...
difference (Assocs kas) (Assocs []) = Assocs kas
difference (Assocs kas) (Assocs ((k', _):kas')) =
    difference (Assocs $ P.filter ((/= k') . fst) kas) (Assocs kas')
...
intersection (Assocs ((k,a):kas)) m2 = case Data.Map.Internal.lookup k m2 of
...
foldrWithKey f z = P.foldr (uncurry f) z . toList
...
fromList kas = Assocs (sby keycmp $ L.nubBy (\(k1, _) (k2, _) -> k1 == k2)
                                 $ reverse kas) 
  where
    keycmp (k1, _) (k2, _) = k1 `compare` k2
...
--                    (_, _) -> error "Failure in Data.Map.Internal.balance"
...
--                      | lrs < ratio*lls -> Bin (1+ls+rs) lk lx ll (Bin (1+rs+lrs) k x lr r)
--                      | otherwise -> Bin (1+ls+rs) lrk lrx (Bin (1+lls+size lrl) lk lx ll lrl) (Bin (1+rs+size lrr) k x lrr r)
--                    (_, _) -> error "Failure in Data.Map.Internal.balanceL"
--               | otherwise -> Bin (1+ls+rs) k x l r
-- {-# NOINLINE balanceL #-}
-- 
...
--                      | rls < ratio*rrs -> Bin (1+ls+rs) rk rx (Bin (1+ls+rls) k x l rl) rr
--                      | otherwise -> Bin (1+ls+rs) rlk rlx (Bin (1+ls+size rll) k x l rll) (Bin (1+rrs+size rlr) rk rx rlr rr)
--                    (_, _) -> error "Failure in Data.Map.Internal.balanceR"
--               | otherwise -> Bin (1+ls+rs) k x l r
-- {-# NOINLINE balanceR #-}
-- 
-- 
